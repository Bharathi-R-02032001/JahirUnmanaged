CLASS lhc__Booking DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR _Booking RESULT result.

    METHODS earlynumbering_cba_Bookingsupp FOR NUMBERING
      IMPORTING entities FOR CREATE _Booking\_Bookingsuppl.

    METHODS Calctotprice FOR DETERMINE ON MODIFY
      IMPORTING keys FOR _Booking~Calctotprice.
...
CLASS lhc__Booking IMPLEMENTATION.
 ...
 READ ENTITIES OF ZI_Travel_Mn IN LOCAL MODE
   ...
   FIELDS ( TravelId booking_status )
   WITH CORRESPONDING #( keys )
   RESULT DATA(lt_booking).

    result  = VALUE #( FOR ls_booking IN lt_booking
                        (  %tky = ls_booking-%tky
                           %features-%assoc-_Bookingsuppl  = COND #( WHEN ls_booking-booking_status = 'X'
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<booking_group>) GROUP BY <booking_group>-%tky.
      ...
      LOOP AT entities ASSIGNING FIELD-SYMBOL(<booking>) USING KEY entity WHERE TravelId  = <booking_group>-TravelId
        ...
        LOOP AT <booking>-%target ASSIGNING FIELD-SYMBOL(<booksuppl_wo_numbers>).
          APPEND CORRESPONDING #( <booksuppl_wo_numbers> ) TO mapped-_booking_supplement ASSIGNING FIELD-SYMBOL(<mapped_booksuppl>).
  ...
  METHOD Calctotprice.
    ...
    DATA: it_travel TYPE STANDARD TABLE OF ZI_Travel_Mn WITH UNIQUE HASHED KEY key COMPONENTS TravelId.
